...
import network.wvae_celeba as vae
import network.vae_celeba as vae_

import functools
from metric import perceptual_path_length as ppl
import utils.wae_config as wae_opts
...
def training_loop(config: Config):
    ...
    adv_solver = tf.train.AdamOptimizer(learning_rate=2 * learning_rate, name='opt', beta1=opts['adam_beta1'])
