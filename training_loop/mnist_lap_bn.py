...
def training_loop(config: Config):
    ...
    def train_step(data):
        ...
        with tf.variable_scope('smooth_loss'):
            s_w, sum_w = smoother_weight(rep, 'heat')
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
        ...
        for iteration in range(config.total_step):
            loss_, r_loss_, kl_loss_, s_loss_, sw_sum_, mse_, lr_ = \
                sess.run([loss, r_loss, kl_loss, s_loss, s_w, mse, learning_rate])
            if iteration % config.print_loss_per_steps == 0:
                timer.update()
                print("step %d, loss %f, r_loss_ %f, kl_loss_ %f, s_loss_ %f, sw_prod %f, mse %f"
                      "learning_rate % f, consuming time %s" %
                      (iteration, loss_, r_loss_, kl_loss_, s_loss_, sw_sum_, mse_,
...
def smoother_weight(y, kernel='heat', margin=0.4):
    ...
    if kernel == 'heat':
        dis = ops.spectral_norm(pairwise_dis)
        w = tf.nn.relu(tf.exp(-dis) - 0.5) * 2
        sum_w = tf.reduce_mean(dis)
