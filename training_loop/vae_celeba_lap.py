...
from datasets.datasets import get_dataset, load_celeba_from_record
...
def training_loop(config: Config):
    ...
    dataset = load_celeba_from_record(config.record_dir + '/CelebA64_rep.tfrecords', config.batch_size)
    ...
    Decoder = vae.Decoder([64, 64, 3], exceptions=['opt'], name='Decoder')
    ...
    def train_step(data):
        ...
        with tf.variable_scope('smooth_loss'):
            s_w, sum_w = smoother_weight(rep, 'heat', sigma2=laplace_sigma2)
        ...
        with tf.control_dependencies([add_global] + update_ops):
            ...
            with tf.control_dependencies([opt]):
                return tf.identity(loss), tf.identity(recon_loss), \
                       tf.identity(kl_divergence), tf.identity(smooth_loss), tf.identity(sum_w)

    loss, r_loss, kl_loss, s_loss, sw_prod = train_step(dataset.get_next())
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
        ...
        for iteration in range(config.total_step):
            loss_, r_loss_, kl_loss_, s_loss_, sw_prod_, lr_ = \
                sess.run([loss, r_loss, kl_loss, s_loss, sw_prod, learning_rate])
            if iteration % config.print_loss_per_steps == 0:
                mse_ = sess.run(mse)
                timer.update()
                print("step %d, loss %f, r_loss_ %f, kl_loss_ %f, s_loss %f, mse %f, sw_prod %f "
                      "learning_rate % f, consuming time %s" %
                      (iteration, loss_, r_loss_, kl_loss_, s_loss_, mse_, np.exp(sw_prod_/config.batch_size),
...
def smoother_weight(y, kernel='heat', margin=None, sigma2=2.5):
    ...
    if kernel == 'heat':
        w = tf.exp(-pairwise_dis / sigma2)
        sum_w = tf.reduce_sum(-pairwise_dis / sigma2)
    else:
